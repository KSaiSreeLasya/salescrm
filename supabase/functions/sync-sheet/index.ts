export default async function handler(req: Request) {
  ...
  try {
    const headerToken =
      req.headers.get("x-sync-token") || req.headers.get("x-api-key") || null;
  ...
  try {
    ...
    sheetUrl =
      (body?.sheet_url as string) || (body?.sheetUrl as string) || undefined;
  } catch (e) {
    // ignore
  }
  ...
  if (!sheetUrl) {
    const cfgRes = await fetch(
      `${SUPABASE_URL.replace(/\/+$/, "")}/rest/v1/config?id=eq.1`,
      {
        headers: { apikey: KEY, Authorization: `Bearer ${KEY}` },
      },
    );
    if (!cfgRes.ok)
      return new Response(`Failed to read config: ${cfgRes.status}`, {
        status: 502,
      });
  ...
  if (!sheetUrl)
    return new Response("No sheet_url configured", { status: 400 });

  // fetch CSV from sheetUrl
  const csvRes = await fetch(sheetUrl);
  if (!csvRes.ok)
    return new Response(`Failed to fetch sheet: ${csvRes.status}`, {
      status: 502,
    });
  ...
  for (const chunkRows of chunk(rows, 100)) {
    ...
    const upsertRes = await fetch(
      `${SUPABASE_URL.replace(/\/+$/, "")}/rest/v1/leads?on_conflict=id`,
      {
        method: "POST",
        headers: {
          apikey: KEY,
    ...
    if (!upsertRes.ok) {
      ...
      return new Response(`Upsert failed: ${upsertRes.status} ${txt}`, {
        status: 502,
      });
    }
  }
